function [RC,SC,UC,EC,TTS, Ill,B,P]=FOM_calculation_demand_based_band_slots_fixed_MODCODs_fixed_B(B_T,N, P_T, Ill, B, P,c_scenario, beams, theta, H, frame, frame_dur, TTL, freq,P_list,sat_idx,K)
RC=0;
SC=0;
UC=0;
EC=0;
...
for i=1:length(c_scenario)
     ...
     c_scenario(i).colour=0;
            for j=1:length(c_scenario(i).users)
                ...
                c_scenario(i).users(j).traffic_served=zeros(1,frame*H);
            end
end

for t=1:frame*H %1frame=10 subframe by 10subframe (beamswithching is performed each frame, for 10 slots constant illumination constant)
    ...
    if mod(t-1,1/frame_dur)==0
        for i=1:length(c_scenario)
            for j=1:length(c_scenario(i).users)
                c_scenario(i).users(j).traffic_pending=c_scenario(i).users(j).traffic_pending+c_scenario(i).users(j).traffic_demand(ceil(t/frame));
            end
        end
    end
    ...
    % NEW: Limit cell_traffic to those cells under the satellite visibility.
    cells_visible=zeros(1,length(c_scenario));
    cells_visible(cell2mat(K(sat_idx,ceil(t/frame))))=1;

    cell_traffic=cell_traffic.*cells_visible;
...
%     plot(polyshape(Footprint_Longitude',Footprint_Latitude'),'FaceColor','yellow','EdgeColor', 'black')
    ...
    %[max2min_ni,idx_max2min_ni]=sort(...
    [max2min,idx_max2min]=sort(cell_traffic,'descend'); % Ojo!! It could happen that the values ordered in max2min do not match with the aggregated pending traffic, as it can be ponderated/weighted depending on the TTL.
    ...
    for beam_idx=1:beams % Select the already ordered and interference limitted beam values.
        if max2min(beam_idx)==0 % If the aggregated traffic if 0 (Could be due to non visibility) then:
            ...
            c_scenario(idx_max2min(beam_idx)).BW(t)=10*log10((sum(B(idx_max2min(beam_idx),:)))*(B_T/N)*10^6); %[dBHz]
            ...
            for u_idx=1:length(c_scenario(idx_max2min(beam_idx)).users)
                if (c_scenario(idx_max2min(beam_idx)).users(u_idx).traffic_pending~=0) % If ==0: no need to serve this user of the cell, the illumination is to serve other demanding users of the cell.
                    ...
                    [minScanning,closestScanningIndex]=min(abs(theta_scanning(1:((np-1)/2+1))+deg2rad(c_scenario(idx_max2min(beam_idx)).betta_to_sat(ceil(t/frame))))); % Gain loss due to scanning
                    G_tx=G_tx+20*log10(cos(abs(theta_scanning(closestScanningIndex)))^q);
                    % FSL:
                    Lfsl=20*log10(freq)+20*log10(c_scenario(idx_max2min(beam_idx)).users(u_idx).distace_to_sat(ceil(t/frame))*10^3)-147.55;
% EXTRA LOSSES!!
...
%                     % Extra_losses.Ag; Extra_losses.Ac; Extra_losses.Ar; Extra_losses.As; Extra_losses.At;
                    ...
                    %   NEW!! BW RX:
                    BW_rx=c_scenario(idx_max2min(beam_idx)).BW(t); %dB
        
                    % CORRECT P_tx as per de MODCOD:
                    
                    [minDistance, indexOfMin] =min(abs(10^(P_tx/10)-P_list(idx_max2min(beam_idx),:,ceil(t/frame))));
                    if P_list(idx_max2min(beam_idx),indexOfMin,ceil(t/frame))>10^(P_tx/10)
                    ...
                    P_tx=10*log10(P_list(idx_max2min(beam_idx),indexOfMin,ceil(t/frame)));
        ...
        %             end
                    ...
                    P(idx_max2min(beam_idx),t)=10^(P_tx/10);
